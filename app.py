...
def render_results(stdout_txt: str, stderr_txt: str, csv_path: str, geo_path: str):
    with result_slot.container():
        st.markdown("### 실행 로그")
        st.code(stdout_txt or "(no stdout)", language="bash")
        st.code(stderr_txt or "(no stderr)", language="bash")
...
if submitted:
    ...
    script_path = (REPO_ROOT / "naver_blog_to_places.py").as_posix()
    st.write("📄 스크립트 경로:", script_path)
    if not os.path.exists(script_path):
        render_results("", "naver_blog_to_places.py를 찾지 못했습니다. 저장소 루트에 파일이 있는지 확인하세요.", "", "")
        st.stop()
