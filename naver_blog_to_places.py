...
def _get_json(sess: requests.Session, url: str, params: Dict[str, Any], max_retries: int = 5) -> Dict[str, Any]:
    """
    Google Places/Geocode용 GET + 지수 백오프.
    - INVALID_REQUEST + next_page_token: 최소 2초 대기 필요
    - OVER_QUERY_LIMIT, RESOURCE_EXHAUSTED 등은 백오프
    """
    backoff = 1.0
    for attempt in range(max_retries):
        ...
        r.raise_for_status()
        js = r.json()
        status = js.get("status") or js.get("Status")  # API 별 케이스
        if status in (None, "OK", "ZERO_RESULTS"):
            return js
        # pageToken를 너무 빨리 쓴 경우
...
def geocode(sess: requests.Session, area: str, country: str) -> Tuple[Optional[float], Optional[float]]:
    ...
    js = _get_json(sess, url, {"address": target, "key": API_KEY, "language": "ko"})
...
def area_pass(fmt: str, area: str, mode: str) -> bool:
    ...
    if any(t.lower() in low for t in toks):
        return True
    if any(AREA_ALIAS.get(t, "").lower() in low for t in toks):
        return True
    return False
...
def normalize_place(p: Dict[str, Any], source_mode: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "lat": loc.get("lat"),
        "lng": loc.get("lng"),
        "business_status": p.get("business_status",""),  # 운영 상태(영업/폐업 등)
        "google_maps_url": f"https://www.google.com/maps/place/?q=place_id:{p.get('place_id','')}",
        "source_mode": source_mode,  # "text" or "nearby" + grid idx 등
    }

def text_search(sess: requests.Session, query, area, country, language, loc, radius_m,
    ...
    for _ in range(max(1, pages)):
        if page_token:
            ...
            time.sleep(2.2)  # 공식 가이드 최소 대기
        js = _get_json(sess, "https://maps.googleapis.com/maps/api/place/textsearch/json", params)
        results = js.get("results", []) or []
        for p in results:
            r = normalize_place(p, "text")
            if not r["place_id"] or r["place_id"] in seen:
                continue
            if not area_pass(r["formatted_address"], area, area_filter):
                continue
            if include_types:
                tset = set((p.get("types") or []))
                if not tset.intersection(set(include_types.split(","))):
                    continue
            if r["rating"] < min_rating or r["user_ratings_total"] < min_reviews:
                continue
            seen.add(r["place_id"])
            out.append(r)
...
def nearby(sess: requests.Session, center_lat, center_lng, area, language, radius_m, grid_steps,
    ...
    out: List[Dict[str, Any]] = []
    # 격자 순회 (항상 동일 순서: 위→아래, 좌→우)
    for lat, lng in grid(center_lat, center_lng, radius_m, grid_steps):
        for tp in include_list:
            params = {
                "location": f"{lat},{lng}", "radius": str(max(1500, radius_m//max(1,grid_steps))),
...
# -------------------- Details 보강 --------------------
def details_enrich(sess: requests.Session, rows: List[Dict[str, Any]], sleep_ms: int) -> List[Dict[str, Any]]:
    # 속도 위해 최소 필드만
...
def main():
    ...
    cap = min(100, max(1, args.max_results))  # 내부 하드 캡(안전)
    seen: Set[str] = set()
    rows: List[Dict[str, Any]] = []

    if args.mode == "text":
        rows = text_search(
            sess, args.query, args.area, args.country, args.language,
        ...
        if not loc:
            raise RuntimeError("Nearby 모드는 중심 좌표가 필요합니다. (지오코딩 실패)")
        rows = nearby(
            sess, loc[0], loc[1], args.area, args.language, args.radius_m,
