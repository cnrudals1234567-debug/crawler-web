...
# ── 본문에서 후보 장소명 추출 ───────────────────────────────────────────
def extract_candidate_pois(text: str, hints, top_k: int = 60):
...
# ── Google Geocoding: 지역 중심좌표 & 국가코드 ───────────────────────────
def geocode_area_country(area: str, country: str):
...
def get_place_country_code(place_id: str):
...
# ── 후보 → 장소 해석 (국가/지역 필터 포함) ───────────────────────────────
def resolve_candidates_to_places(
    candidates, area, country="",
    language="ko", loc_bias=None, radius_m=None, sleep_ms=300,
    target_country_code=None, require_area_in_address=True
):
    out = []
        ...
        try:
            ...
            if require_area_in_address and area_tokens:
                ...
                if not any(tok.lower() in lowered for tok in area_tokens):
                    # 지역명이 영문/현지어로 표기될 수 있음 → 필요하면 사전 확장
                    pass  # 엄격 필터가 필요하면 여기서 continue로 제외 가능

            # 국가코드 검사 (가장 확실)
            cc = get_place_country_code(top.get("place_id", ""))
            if target_country_code and cc and cc != target_country_code:
                time.sleep(max(0, sleep_ms) / 1000.0)
                continue
...
def write_geojson(rows, path):
    ...
    for r in rows:
        if r.get("lat") is None or r.get("lng") is None:
            continue
        props = dict(r)
        # lat/lng는 geometry에 있으니 properties에서는 제거해도 됨(선택)
        feats.append({
            "type": "Feature",
...
def main():
    ...
    ap.add_argument("--hint_mode", default="query")       # fixed/query/both/none 중 필요시 확장
    ap.add_argument("--language", default="ko")
    ap.add_argument("--radius_m", type=int, default=10000)  # 10km
    ap.add_argument("--max_posts", type=int, default=30)     # 최대 50 추천
    ...
    ap.add_argument("--no_cache", action="store_true")    # 현재 로직에서는 캐시 미사용
    ap.add_argument("--log_urls", action="store_true")    # 블로그 URL 로그 저장 여부
    args = ap.parse_args()
    ...
    for h in hints:
        hl = h.lower()
        if hl not in seen:
            seen.add(hl); eff_hints.append(h)
