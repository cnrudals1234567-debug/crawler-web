import re, json, time, csv, argparse, os
...
def extract_main_text(html:str)->str:
    try:
        doc = Document(html); soup = BeautifulSoup(doc.summary(), "html.parser")
    except Exception:
...
def extract_candidate_pois(text:str, top_k:int=60):
    ...
    cleaned=[]
    for c in cand:
        c=re.sub(r"\s{2,}"," ",c); c=re.sub(r"^\d+\.\s*","",c).strip()
        if c.lower() not in ban and len(c)<=40: cleaned.append(c)
    uniq=[]; seen=set()
    for c in cleaned:
        k=c.lower()
...
def write_csv(rows, path):
    cols=["candidate_name","resolved_name","formatted_address","place_id","rating","user_ratings_total","types","lat","lng","google_maps_url","source_url"]
...
def write_geojson(rows, path):
    ...
    for r in rows:
        ...
        feats.append({"type":"Feature","geometry":{"type":"Point","coordinates":[r["lng"],r["lat"]]},
                      "properties":{"name":r.get("resolved_name") or r.get("candidate_name"),
...
def main():
    ...
    print(f"[INFO] Searching Naver blogs: {args.query}")
    ...
    for url in urls:
        ...
        try:
            ...
            print(f"[WARN] Fetch failed: {url} | {e}"); continue
    ...
    out_base=slugify(f"{args.city}-{args.query}")
    csv_path=f"{out_base}.csv"; geo_path=f"{out_base}.geojson"
    write_csv(list(uniq.values()), csv_path)
    write_geojson(list(uniq.values()), geo_path)
